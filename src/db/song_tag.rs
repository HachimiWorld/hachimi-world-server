...
where E: PgExecutor<'e> {
    async fn list_by_ids(executor: E, ids: &[i64]) -> sqlx::Result<Vec<SongTag>> {
        ...
        query_song_tags!("WHERE id = ANY($1)", ids)
    ...
    async fn get_by_name(executor: E, name: &str) -> sqlx::Result<Option<SongTag>> {
        query_song_tags!("WHERE name = $1", name)
            .fetch_optional(executor)
            .await
    }
    ...
    async fn search_by_prefix(executor: E, prefix: &str) -> sqlx::Result<Vec<SongTag>> {
        let mut escaped = prefix.replace("%", "\\%")
            .replace("_", "\\_");
        escaped.push_str("%");
        query_song_tags!("WHERE name LIKE $1 LIMIT 20", escaped)
            .fetch_all(executor)
            .await
    }
    ...
    async fn get_random_tags(executor: E, limit: i64) -> sqlx::Result<Vec<SongTag>> {
        // Table sampling
        query_song_tags!("TABLESAMPLE SYSTEM_ROWS($1)", limit)
            .fetch_all(executor)
            .await
    }
}
